...
public void
terrain (int tmp, int x, int y)
{
  if (simulateButton)
    {
      PVector vector = new PVector (Math.round (x * 100) / 100.00,
                                    Math.round (y * 100) / 100.00);
      ...
      if (map.get (vector) == null)
        ...
        {
          total = map.get (vector);
          map.put (vector, ++total);
        }
      ...
      if (total < 4)
        ...
        { // snow
          fill (total * 20, total * 20, total * 20);
        }
    }
}
}
